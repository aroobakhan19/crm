...
const ViewOfficeAvabilityDescription = () => {
    const { id } = useParams()
    const [viewAvability, setViewAvability] = useState([]);

    useEffect(() => {
        getofficeAvabilityDescription();
    },[]);
    
    async function getofficeAvabilityDescription() {
      try {
        const res = await fetch(`https://crm-backend-plum.vercel.app/officeAvability/${id}`);
  ...
  return (
    ...
    >
      ...
      {viewAvability.requirmentType === 'Rent' ?
...
export default ViewOfficeAvabilityDescription
